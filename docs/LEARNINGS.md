...
- Custom template-support sockets fit cleanly in ComfyUI V3 via `IO.Custom(...)`; for mustache variables, a useful payload shape is a list of one-entry dicts like `[{"name": "Ada"}]`, which keeps later list concatenation/merge behavior straightforward.
