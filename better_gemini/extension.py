    ...
    *,
    ...
    for idx, img_bytes in enumerate(images):
        with Image.open(BytesIO(img_bytes)) as img:
            img = img.convert("RGB")
            w, h = img.size
            mismatch = resolution_mismatch_message(
                actual_width=w,
                actual_height=h,
                requested_resolution=requested_resolution,
                requested_width=requested_width,
            ...
            return IO.Schema(
                ...
                inputs=[
                    ...
                    IO.Combo.Input(
                        ...
                        default="auto",
                        tooltip="Requested output resolution (best-effort; may be ignored by the model). Logs a warning if the returned image size doesn't match.",
                        optional=True,
                    ),
                    IO.Int.Input(
                        "width",
            ...
            return IO.Schema(
                ...
                inputs=[
                    ...
                    IO.Combo.Input(
                        ...
                        options=["IMAGE", "IMAGE+TEXT", "TEXT"],
                        default="IMAGE",
                        tooltip="Choose IMAGE output for Grok image generation, IMAGE+TEXT to keep image generation plus node notes, or TEXT to call Grok responses and return the result through STRING.",
                    ),
                    IO.Combo.Input(
                        "reasoning_effort",
                        options=["none", "low", "medium", "high"],
                        default="none",
                        tooltip="Reasoning effort for Grok text requests. Image generation ignores this setting.",
                        optional=True,
                    ),
                    IO.Image.Input(
                        "prompt_images",
