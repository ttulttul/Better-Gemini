...
class FrontendContractTests(unittest.TestCase):
    def test_frontend_adds_node_chrome_login_logout_and_never_handles_tokens(self):
        ...
        self.assertIn('node.addWidget?.("button", buttonLabel()', source)
        self.assertIn("scheduleRendererButton(node)", source)
        self.assertIn("async init()", source)
        self.assertIn("await initializeFrontend()", source)
